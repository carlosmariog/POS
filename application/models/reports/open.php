...
class Open extends Report
{
	...
	public function getDeposit($workstation){
		$this->db->from('workstation');
		$this->db->where('workstation_id', $workstation);
		$this->db->group_by("workstation_id");
		$query = $this->db->get();
		foreach ($query->result() as $row)
		{
			$cash  = $row->cash_deposit;
			$check = $row->check_deposit;
			return ($cash+$check);
		}
		return (0);
	}
