...
class Summary_customers extends Report
{
	...
	{
		$this->db->select('CONCAT(ospos_people.first_name, " ",ospos_people.last_name) as customer, sum(subtotal) as subtotal, sum(total) as total, sum(tax) as tax,sum(profit) as profit', false);
		...
		$this->db->order_by('ospos_people.last_name');
