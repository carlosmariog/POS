...
class Specific_customer extends Report
{
	...
	{
		$this->db->select('sale_id, sale_date, sum(quantity_purchased) as items_purchased, CONCAT(ospos_people.first_name," ",ospos_people.last_name) as employee_name, sum(subtotal) as subtotal, sum(total) as total, sum(tax) as tax, sum(profit) as profit, payment_type, comment', false);
		...
		{
			$this->db->select('sales_items_temp.name, category_id, serialnumber, sales_items_temp.description, quantity_purchased, subtotal,total, tax, profit, discount_percent');
			$this->db->from('sales_items_temp');
