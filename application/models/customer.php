...
{	
	...
	*/
	...
	function anonymousExists()
	{
		$this->db->from('customers');	
		$this->db->join('people', 'people.person_id = customers.person_id');
		$this->db->where('people.first_name','Anonymous');
		$this->db->where('people.last_name','Anonymous');
		$query = $this->db->get();
		return ($query->num_rows()==1);
	}

	function updateDesiredItems($person_id){
		$this->db->from('people');
		$data = array(
               'desired_items' => '');
	...
	*/
	...
	function getBanned($customer_id){
		$this->db->from('people');
		$this->db->where('person_id',$customer_id);
		$this->db->limit(1);		
		return $this->db->get()->row()->banned;
	}
	
	function getDesiredItems(){
		$this->db->select('first_name, last_name, phone_number, email, desired_items');
		$this->db->from('people');
		$this->db->where('desired_items !=','');
		return $this->db->get();
	}
