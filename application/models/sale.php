...
class Sale extends CI_Model
{
	...
	public function create_sales_items_temp_table()
	{
		...
		(SELECT date(sale_time) as sale_date, first_name, last_name, address_1, phone_number, identification_type, ".$this->db->
		dbprefix('people').".person_id, identification, ".$this->db->dbprefix('sales_items').".sale_id, comment,payment_type, 
		customer_id, ".$this->db->dbprefix('items').".item_number, employee_id, ".$this->db->dbprefix('items').".item_id,".$this->db->dbprefix('items').".name,".$this->db->
		dbprefix('items').".store_id,"."supplier_id, quantity_purchased, item_cost_price, item_unit_price, SUM(percent) as 
		item_tax_percent,discount_percent, (item_unit_price * quantity_purchased - item_unit_price * quantity_purchased *
		 discount_percent / 100) as subtotal,".$this->db->dbprefix('sales_items').".line as line, serialnumber, ".$this->db->
		 dbprefix('sales_items').".description as description,
		...
		INNER JOIN ".$this->db->dbprefix('people')." ON  ".$this->db->dbprefix('sales').'.customer_id='.$this->db->dbprefix('people').'.person_id'."
	...
	public function isCoveredbyWarrantyRepair($item_number){
		$actual_repair_data = date('Y-m-d h:i:s');
		...
		foreach ($query->result() as $row)
		{
			...
			$actual_year=substr($actual_repair_data,0,4);
			$warranty_year=substr($sale_time,0,4);
			if(($actual_year-$warranty_year)>0)
				return false;
			$actual_month = substr($actual_repair_data,5,2);
			$warranty_month = substr($sale_time,5,2);
			if(($actual_month-$warranty_month)>9)
				return false;
			else
				return true;
		}
	...
	public function isCoveredbyWarrantySales($item_number){
		$actual_repair_data = date('Y-m-d h:i:s');
		$this->db->select('sale_time');
		$this->db->from('sales_items');
		$this->db->join('sales', 'sales_items.sale_id = sales.sale_id');
		$this->db->join('items', 'sales_items.item_id = items.item_id');
		$this->db->where('item_number', $item_number); 
		$this->db->where('register_mode', 'Sales'); 
		$this->db->order_by("sale_time", "desc");
		$this->db->limit(1);
		$query = $this->db->get();
		foreach ($query->result() as $row)
		{
			...
			$actual_year=substr($actual_repair_data,0,4);
			$warranty_year=substr($sale_time,0,4);
			if(($actual_year-$warranty_year)>1)
				return false;
			else
				return true;
		}
	...
	public function deposit($deposit_info){
			$this->db->insert('workstation',$deposit_info);
	}
