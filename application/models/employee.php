...
class Employee extends Person
{
	...
	*/
	...
	function update_session_id($session_id, $person_id){
		$data = array(
	...
	function get_session_id($person_id){
		$this->db->select('sessionid');
		$this->db->from('employees');	
		$this->db->where('employees.person_id',$person_id);
		return $this->db->get()->row()->sessionid;		
	}
	...
	function update_end_date($username){
	...
	*/
	function login($username, $password)
	{
		...
		if ($query->num_rows() ==1)
		{
		   ...
		   /*$islogged = $this->db->get_where('employees_tracking', array('person_id' => $row->person_id,'end_session'=>'0000-00-00
		    00:00:00'), 1);		
			if ($islogged->num_rows() >=1)
			{
				//Is already logged
				return 1;
			}*/
			...
			$this->session->set_userdata('start_session', $startDate);
			
			$data = array(
               'person_id' => $row->person_id,
               'start_session' => $startDate
            );
			$this->db->insert('employees_tracking', $data);
	...
	*/
	function logout()
	{
		...
		$startDate= $this->session->userdata('start_session');
		$query = $this->db->query("UPDATE ospos_employees_tracking SET end_session='".$endDate."' WHERE start_session='".$startDate."' AND end_session='0000-00-00 00:00:00' AND person_id=".$this->session->userdata('person_id')." LIMIT 1;");
