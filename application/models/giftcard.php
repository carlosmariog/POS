...
class Giftcard extends CI_Model
{
	...
	*/
	...
	Determines if a given giftcard_id is an giftcard
	*/
	function existsGCnumber( $giftcard_number )
	{
		$this->db->from('giftcards');
	...
	Determines if a given giftcard_id is an giftcard
	*/
	function existsGCnumberDelete( $giftcard_number )
	{
		$this->db->from('giftcards');
		$this->db->where('giftcard_number',$giftcard_number);
	...
	Determines if a given giftcard_id is an giftcard
	*/
	function existsGCnumberActive( $giftcard_number )
	{
		$this->db->from('giftcards');
		$this->db->where('giftcard_number',$giftcard_number);
...
/*
	Get an giftcard id given an giftcard number
	*/
	function get_giftcard_number($giftcard_id)
	{
		$this->db->from('giftcards');
		$this->db->where('giftcard_id',$giftcard_id);
		$this->db->where('deleted',0);
		...
		if($query->num_rows()==1)
		{
			return $query->row()->giftcard_number;
		}
	...
	*/
	...
	Inserts or updates a giftcard
	*/
	function addCredit($giftcard_number,$giftcard_value, $giftcard_id)
	{
		$giftcard_da = array(
				'value'=>$this->get_giftcard_value( $giftcard_number ) + $giftcard_value
		);
		$this->db->where('giftcard_id', $giftcard_id);
		return $this->db->update('giftcards',$giftcard_da);
	}
	
	/*
	Inserts or updates a giftcard
	*/
	function subtractCredit($giftcard_number,$giftcard_value, $giftcard_id)
	{
		$giftcard_da = array(
				'value'=>$this->get_giftcard_value( $giftcard_number ) - $giftcard_value
		);
	...
	*/
	function deleteGift($giftcard_id, $value)
	{
		if ($this->get_giftcard_value($giftcard_id)== $value)
		{
			$this->db->delete('giftcards', array('giftcard_id' => $giftcard_id));	
		}
		else{
			$this->subtractCredit($this->get_giftcard_number($giftcard_id),$value, $giftcard_id);
		}
	}
	...
	Deletes one giftcard
	*/
	function delete($giftcard_id, $value)
	{
		return $this->db->delete('giftcards', array('giftcard_id' => $giftcard_id));
	}
	...
	*/
	...
	function update_giftcard_value_deleted_to_active( $giftcard_number, $value )
	{
		$this->db->where('giftcard_number', $giftcard_number);
		$this->db->update('giftcards', array('value' => $value,'delete' => 0));
	}
