<?php
class Used_istem extends CI_Model
{
	/*
	...
	*/
	function get_info($item_id)
	{
		...
		{
			//Get empty base parent object, as $item_id is NOT an used item
			$item_obj=new stdClass();

			//Get all the fields from used items table
			$fields = $this->db->list_fields('used_items');
	...
	*/
	function save(&$item_data,$item_id=false)
	{
		...
		{
			if($this->db->insert('used_items',$item_data))
			{
		...
		$this->db->where('item_id', $item_id);
		return $this->db->update('used_items',$item_data);
	}
	...
	*/
	...
	{
		$this->db->where('item_id', $item_id);
		return $this->db->update('used_items', array('deleted' => 1));
	}
	...
	*/
	function delete_list($item_ids)
	{
 	...
 	/*
	Get search suggestions to find used items
	*/
