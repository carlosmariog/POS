...
{	
	...
	function summary_categories($start_date, $end_date, $sale_type, $export_excel=0)
	{
		...
		foreach($report_data as $row)
		{
			$tabular_data[] = array($this->Item->get_category_name($row['category_id']), to_currency($row['subtotal']), to_currency($row['total']), to_currency($row['tax']),to_currency($row['profit']));
		}
	...
	function summary_items($start_date, $end_date, $sale_type, $export_excel=0)
	{
		...
		foreach($report_data as $row)
		{
			$tabular_data[] = array($row['serialnumber'], $row['item_number'],character_limiter($row['name'], 16), $row['quantity_purchased'], to_currency($row['subtotal']), to_currency($row['total']), to_currency($row['tax']),to_currency($row['profit']));
		}
	...
	function graphical_summary_categories_graph($start_date, $end_date, $sale_type)
	{
		...
		foreach($report_data as $row)
		{
			$graph_data[$this->Item->get_category_name($row['category_id'])] = $row['total'];
		}
	...
	function specific_employee_tracking_input()
	{
		$data = $this->_get_common_report_data();
		$data['specific_input_name'] = $this->lang->line('reports_employee');
		...
		$data['specific_input_data'] = $employees;
		$this->load->view("reports/employees_tracking",$data);	
	}


	function specific_employee_tracking($start_session, $end_session, $employee_id, $export_excel=0)
	{	
		$this->load->model('reports/Specific_employee_tracking');
		$model = $this->Specific_employee_tracking;
		
		$headers = $model->getDataColumns();
		$report_data = $model->getData(array('start_session'=>$start_session, 'end_session'=>$end_session, 'employee_id' =>$employee_id));	

		$summary_data = array();
		foreach($report_data['summary'] as $key=>$row)
		{
			$start_data=$row['start_session'];
			$end_data=$row['end_session'];
			//$years=substr($start_data,0,4)-substr($end_data,0,4);
			//$months=substr($start_data,5,2)-substr($end_data,5,2);
			//$days=substr($start_data,8,2)-substr($end_data,8,2);
			$hours=substr($end_data,11,2)-substr($start_data,11,2);
			$minutes=substr($end_data,14,2)-substr($start_data,14,2);
			if($minutes<0){
				$minutes=(60-substr($start_data,14,2))+substr($end_data,14,2);
				$hours=$hours-1;
			}
			...
			$summary_data[] = array($row['start_session'], $row['end_session'], $time);
		}
				
		$employee_info = $this->Employee->get_info($employee_id);
		$data = array(
			"title" => $employee_info->first_name .' '. $employee_info->last_name.' '.$this->lang->line('reports_report'),
			"subtitle" => date('m/d/Y', strtotime($start_session)) .'-'.date('m/d/Y', strtotime($end_session)),
			"headers" => $headers,
			"summary_data" => $summary_data,
			"export_excel" => $export_excel
		);
		
		$this->load->view("reports/employee_tracking_details",$data);		
	}


	//Summary sales report
	function summary_police($start_date, $end_date, $sale_type, $export_excel=0)
	{
		$this->load->model('reports/Summary_police');
		$model = $this->Summary_police;
		...
		foreach($report_data['summary'] as $key=>$row)
		{	
			$summary_data[] = array($row['sale_date'], $row['first_name'], $row['last_name'], $row['address_1'] ,$row['phone_number'] , $row['identification_type'], $row['identification'], $row['sale_id'], $row['item_id'], $row['name'], $row['serialnumber'], to_currency($row['total']));
		}
		
		$data = array(
			"title" => $this->lang->line('reports_police_summary_report'),
			"subtitle" => date('m/d/Y', strtotime($start_date)) .'-'.date('m/d/Y', strtotime($end_date)),
			"headers" => $model->getDataColumns(),
			"summary_data" => $summary_data,
			"export_excel" => $export_excel
		...
		$this->load->view("reports/tabular_police",$data);
	}
	...
	function detailed_receivings($start_date, $end_date, $sale_type, $export_excel=0)
	{
		...
		foreach($report_data['summary'] as $key=>$row)
		{
			...
			foreach($report_data['details'][$key] as $drow)
			{
				$details_data[$key][] = array($drow['name'], $drow['category_id'], $drow['quantity_purchased'], to_currency($drow['total']), $drow['discount_percent'].'%');
			}
		}
	...
	function reconcile(){
		...
		$this->load->model('reports/Reconcile');
		$model = $this->Reconcile;
		...
		$data = array(
			"title" =>$this->lang->line('reports_reconcile'),
			"subtitle" => date('m/d/Y', strtotime($start_date)) .'-'.date('m/d/Y', strtotime($end_date)),
			"headers" => $model->getDataColumns(),
			"summary_data" => $summary_data,
			"details_data" => $details_data,
