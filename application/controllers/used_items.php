...
class Used_items extends Secure_area implements iData_controller
{
	function __construct()
	{
		parent::__construct('used_items');
	}

	/*function index()
	{
		$this->load->view('used_items/used_items_view');
	}*/
	
	function index()
	{
		$config['base_url'] = site_url('/Used_items/index');
		...
		$data['manage_table']=get_used_items_manage_table( $this->Item->get_all( $config['per_page'], $this->uri->segment( $config[			'uri_segment'] ) ), $this );
		$this->load->view('used_items/manage',$data);
	}
	...
	*/
	...
	function view($item_id=-1)
	{
		...
		$suppliers = array('' => $this->lang->line('used_items_none'));
		...
		$this->load->view("used_items/form",$data);
	}
	...
	{
		$data['item_info']=$this->Item->get_info($item_id);
		$this->load->view("used_items/inventory",$data);
	}
	
	function count_details($item_id=-1)
	{
		$data['item_info']=$this->Item->get_info($item_id);
		$this->load->view("used_items/count_details",$data);
	} //------------------------------------------- Ramel
	...
	function bulk_edit()
	{
		...
		$data['allow_alt_desciption_choices'] = array(
			''=>$this->lang->line('used_items_do_nothing'), 
			1 =>$this->lang->line('used_items_change_all_to_allow_alt_desc'),
			0 =>$this->lang->line('used_items_change_all_to_not_allow_allow_desc'));
				
		$data['serialization_choices'] = array(
			''=>$this->lang->line('used_items_do_nothing'), 
			1 =>$this->lang->line('used_items_change_all_to_serialized'),
			0 =>$this->lang->line('used_items_change_all_to_unserialized'));
		$this->load->view("used_items/form_bulk", $data);
	}
	...
	function save($item_id=-1)
	{
		...
		if($this->Item->save($item_data,$item_id))
		{
			...
			if($item_id==-1)
			{
				echo json_encode(array('success'=>true,'message'=>$this->lang->line('used_items_successful_adding').' '.
				$item_data['name'],'item_id'=>$item_data['item_id']));
				$item_id = $item_data['item_id'];
			}
			else //previous item
			{
				echo json_encode(array('success'=>true,'message'=>$this->lang->line('used_items_successful_updating').' '.
			...
			(
				...
				'trans_comment'=>$this->lang->line('used_items_manually_editing_of_quantity'),
		...
		{
			echo json_encode(array('success'=>false,'message'=>$this->lang->line('used_items_error_adding_updating').' '.
	...
	function bulk_update()
	{
		...
		if(empty($item_data) || $this->Item->update_multiple($item_data,$items_to_update))
		{
			...
			echo json_encode(array('success'=>true,'message'=>$this->lang->line('used_items_successful_bulk_edit')));
		}
		else
		{
			echo json_encode(array('success'=>false,'message'=>$this->lang->line('used_items_error_updating_multiple')));
		}	
	}
	...
	function delete()
	{
		...
		{
			echo json_encode(array('success'=>true,'message'=>$this->lang->line('used_items_successful_deleted').' '.
			count($items_to_delete).' '.$this->lang->line('used_items_one_or_multiple')));
		}
		else
		{
			echo json_encode(array('success'=>false,'message'=>$this->lang->line('used_items_cannot_be_deleted')));
		}	
	}
	...
	{
		$this->load->view("used_items/excel_import", null);	
	}
	...
	function do_excel_import()
	{
		...
		{
			$msg = $this->lang->line('used_items_excel_import_failed');
