...
function get_stores_manage_table($stores,$controller)
{
	...
	$CI->lang->line('config_name_store'),
	$CI->lang->line('config_address_store'),
	$CI->lang->line('config_store_workstation'),
	$CI->lang->line('config_phone_store'),
	$CI->lang->line('config_store_email'),
	$CI->lang->line('config_store_fax'),
	$CI->lang->line('config_store_timezone'),
	'&nbsp');
	...
	$table.=get_store_manage_table_data_rows($stores,$controller);
	$table.='</tbody></table>';
	return $table;
}
...
*/
function get_store_manage_table_data_rows($stores,$controller)
{
	$CI =& get_instance();
	$table_data_rows='';
	...
	{
		$table_data_rows.="<tr><td colspan='6'><div class='warning_message' style='padding:7px;'>".$CI->lang->line('common_no_stores_to_display')."
		</div></tr></tr>";
	}
...
//	$CI->lang->line('config_store_timezone'),
	...
	$table_data_row='<tr>';
	$table_data_row.="<td width='5%'><input type='checkbox' id='store->store_id' value='".$store->store_id."'/></td>";
	$table_data_row.='<td width="20%"><div>'.character_limiter($store->store_name,18).'</td>';
	$table_data_row.='<td width="20%"><div>'.character_limiter($store->store_address,18).'</td>';
	$table_data_row.='<td width="20%"><div>'.character_limiter($store->workstation,18).'</td>';
	$table_data_row.='<td width="20%"><div>'.character_limiter($store->phone,13).'</div></td>';		
	$table_data_row.='<td width="30%">'.mailto($store->store_email,character_limiter($store->store_email,22), array('target'  => '_blank')).'</td>';
	$table_data_row.='<td width="20%"><div>'.character_limiter($store->store_fax,13).'</div></td>';	
	$table_data_row.='<td width="20%"><div>'.character_limiter($store->store_timezone,13).'</div></td>';		
		
	$table_data_row.='<td width="5%">'.anchor($controller_name."/view/$store->store_id/width:$width", $CI->lang->line('common_edit'),array('class'=>'thickbox','title'=>$CI->lang->line($controller_name.'_update'))).'</td>';		
	$table_data_row.='</tr>';
...
function get_people_manage_table_data_rows($people,$controller)
{
	...
	{
		$table_data_rows.="<tr><td colspan='6'><div class='warning_message' style='padding:7px;'>".$CI->lang->line('common_no_persons_to_display')."
		</div></tr></tr>";
	}
...
function get_person_data_row($person,$controller)
{
	...
	$table_data_row.='<td width="20%"><div>'.character_limiter($person->last_name,13).'</td>';
	$table_data_row.='<td width="20%"><div>'.character_limiter($person->first_name,13).'</td>';
	$table_data_row.='<td width="30%">'.mailto($person->email,character_limiter($person->email,22), array('target'  => '_blank')).'</td>';
	$table_data_row.='<td width="20%"><div>'.character_limiter($person->phone_number,13).'</div></td>';		
...
function get_supplier_data_row($supplier,$controller)
{
	...
	$table_data_row.='<td width="17%"><div>'.character_limiter($supplier->company_name,13).'</div></td>';
	$table_data_row.='<td width="17%"><div>'.character_limiter($supplier->last_name,13).'</div></td>';
	$table_data_row.='<td width="17%"><div>'.character_limiter($supplier->first_name,13).'</div></td>';
	$table_data_row.='<td width="22%">'.mailto($supplier->email,character_limiter($supplier->email,22)).'</td>';
	$table_data_row.='<td width="17%"><div>'.character_limiter($supplier->phone_number,13).'</div></td>';		
...
function get_items_manage_table($items,$controller)
{
	...
	/*$table = '<script type="text/javascript" src="http://ajax.googleapis.com/ajax/libs/prototype/1.6.0.2/prototype.js"></script>
			  <script type="text/javascript" src="js/fabtabulous.js"></script>
			  <script type="text/javascript" src="js/tablekit.js"></script>';*/
			  
	//$table.='<table class="tablesorter sortable resizable editable" id="sortable_table">';
	...
	$CI->lang->line('items_barcode_quantity'),	
	$CI->lang->line('items_price_check'),
	'&nbsp;',
...
/*
Gets the html table to manage used items.
*/
function get_used_items_manage_table($items,$controller)
{
	$CI =& get_instance();
	$table='<table class="tablesorter" id="sortable_table">';
	
	$headers = array('<input type="checkbox" id="select_all" />', 
	$CI->lang->line('used_items_item_number'),
	$CI->lang->line('used_items_name'),
	$CI->lang->line('used_items_category'),
	$CI->lang->line('used_items_cost_price'),
	$CI->lang->line('used_items_unit_price'),
	$CI->lang->line('used_items_tax_percents'),
	$CI->lang->line('used_items_quantity'),
	'&nbsp;',
	$CI->lang->line('used_items_average'),
	$CI->lang->line('used_items_inventory')
	);
	...
	$table.='</tr></thead><tbody>';
	$table.=get_used_items_manage_table_data_rows($items,$controller);
	$table.='</tbody></table>';
	return $table;
}
...
/*
Gets the html data rows for the used items.
*/
function get_used_items_manage_table_data_rows($items,$controller)
{	
	$CI =& get_instance();
	$table_data_rows='';
	...
	foreach($items->result() as $item)
	{
		$table_data_rows.=get_used_item_data_row($item,$controller);
	}
...
function get_used_item_data_row($item,$controller)
{
	$CI =& get_instance();
	...
	$table_data_row.='<td width="14%">'.$CI->Item->get_category_name($item->category_id).'</td>';
	...
	$table_data_row.='<td width="14%"><input name="average" type="text" />'./*$item->average.*/'</td>';
...
function get_item_data_row($item,$controller)
{
	...
	//$start_data;
	$start_data = $CI->Inventory->get_last_inventory_trans_data_for_item($item->item_id);
	$end_data= date('Y-m-d H:i:s');
	$price_check = false;
	$years = substr($end_data,0,4)-substr($start_data,0,4);
	$months = substr($end_data,5,2)-substr($start_data,5,2);
	
	if($years>0)	$price_check = true;
	elseif($months>0) $price_check = true;


	$table_data_row='<tr>';
	$table_data_row.="<td width='3%'><input type='checkbox' id='$item->item_id' value='".$item->item_id."'/></td>";
	$table_data_row.='<td width="15%"><div class="text" id="number_'.$item->item_id.'">'.$item->item_number.'</div></td>';
	$table_data_row.='<td width="20%"><div class="text" id="name_'.$item->item_id.'">'.$item->name.'</div></td>';
	$table_data_row.='<td width="14%"><div class="text" id="category_'.$item->item_id.'">'.$CI->Item->get_category_name($item->category_id).'</div></td>';
	$table_data_row.='<td width="14%"><div class="text" id="cost_'.$item->item_id.'">'.to_currency($item->cost_price).'</div></td>';
	$table_data_row.='<td width="14%"><div class="text" id="unit_'.$item->item_id.'">'.to_currency($item->unit_price).'</div></td>';
	$table_data_row.='<td width="14%"><div id="tax_'.$item->item_id.'">'.$tax_percents.'</div></td>';	
	$table_data_row.='<td width="14%"><div id="quantity_'.$item->item_id.'">'.$item->quantity.'</div></td>';
	$table_data_row.='<td width="14%"><div class="text" id="barcodes_'.$item->item_id.'">'.$item->barcodes_to_generate.'</div></td>';


	if($price_check){
		$table_data_row.='<td width="14%"><img src="'.base_url().'images/check.png" border="0" alt="check" />'.'</td>';
	}else{
		$table_data_row.='<td width="14%"></td>';
	}
